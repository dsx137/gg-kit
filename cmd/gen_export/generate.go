...
func gen(p *packages.Package, alias string) []byte {
	ign := ignores(p)
	scope := p.Types.Scope()
	...
	// Types we re-export via `type X = alias.X`
...
func wrapFunc(name string, sig *types.Signature, alias string, tw *typeWriter) (decl string, stmt string) {
	...
	// Call: include explicit type args only when inference from args is impossible.
